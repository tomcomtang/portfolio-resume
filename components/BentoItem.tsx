...
export function useFadeIn() {
  ...
  // Important: keep initial render identical between server and client
  // to avoid hydration mismatch.
  const [style, setStyle] = useState<CSSProperties | undefined>(undefined);

  useEffect(() => {
    // If animations are enabled, start hidden on client after mount.
    if (document.documentElement.dataset.animate) {
...
export function BentoItem({
  ...
  return (
    <div
      className={twMerge(
        "relative flex flex-col justify-end rounded-3xl overflow-hidden col-span-full px-4 sm:px-6 md:px-8 lg:px-10 py-6 md:py-8 bg-slate-300 dark:bg-slate-700 ring-1 ring-black/10 shadow shadow-black/20",
